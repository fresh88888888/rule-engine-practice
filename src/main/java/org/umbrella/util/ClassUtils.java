...
import static org.umbrella.util.MethodUtils.getMethod;
import static org.umbrella.util.StringUtils.ucFirst;

public final class ClassUtils {
    private static final Map<String, Class<?>> classes = synchronizedMap(new HashMap());

    private static final Map<String, Constructor<?>> constructors = synchronizedMap(new HashMap());
    ...
    private static final Set<Class<?>> numericClasses = new HashSet<>(
            asList(int.class, long.class, double.class, float.class, short.class, char.class, byte.class,
    ...
    /**
     * This method will attempt to create an instance of the specified Class. It
     * uses
     * a syncrhonized HashMap to cache the reflection Class lookup.
     */
    public static Object instantiateObject(String className,
     ...
     * This method will attempt to create an instance of the specified Class. It
     * uses
     * a synchronized HashMap to cache the reflection Class lookup. It will execute
     * the default
    ...
    /**
     * Determines if a given full qualified class name matches any import style
     * patterns.
     */
    ...
    public static String getPackage(Class<?> cls) {
        // cls.getPackage() sometimes returns null, in which case fall back to string
        // massaging.
    ...
    private static Method bestCandidateAccessor(Class<?> clazz, List<Method> accessors,
            Map<String, Integer> accessorPriorityMap, boolean exceptionIfIncompatible) {
            ...
            if (bestCandidate != null && !MethodUtils.isOverride(bestCandidate, method)) {
                if (method.getReturnType() != bestCandidate.getReturnType()) {
                    if (method.getReturnType().isAssignableFrom(bestCandidate.getReturnType())) {
                        // a more specialized getter (covariant overload) has been already indexed, so
                        // skip this one
                        continue;
                        ...
                        && Modifier.isAbstract(bestCandidate.getModifiers())) {
                    // If both are abstract, no need of Warning
                } else if (accessorPriorityMap.get(bestCandidate.getName()) > accessorPriorityMap
                        .get(method.getName())) {
    ...
    public static Method getSetter(Class<?> clazz, String field, Class<?> parameterType) {
        return Stream.<Supplier<String>>of(
                ...
                .orElse(parameterType.isPrimitive() ? getSetter(clazz, field, convertFromPrimitiveType(parameterType))
                        : null);
    }
    ...
    public static Class extractGenericType(Class<?> clazz, final String methodName) {
        ...
        if (returnType instanceof ParameterizedType) {
            ...
            if (typeArguments.length > 0) {
                return typeArguments[0] instanceof ParameterizedType
                        ? (Class) ((ParameterizedType) typeArguments[0]).getRawType()
    ...
    public static Type getTypeArgument(Type genericType, int index) {
        return genericType instanceof ParameterizedType
                ? ((ParameterizedType) genericType).getActualTypeArguments()[index]
                : Object.class;
    }
    ...
    public static Class<?> getClassFromName(String className, boolean initialize, ClassLoader classLoader)
    ...
    public static String getCanonicalSimpleName(Class<?> c, char separator) {
        ...
        return enclosingClass != null ? getCanonicalSimpleName(enclosingClass) + separator + c.getSimpleName()
                : c.getSimpleName();
    }
