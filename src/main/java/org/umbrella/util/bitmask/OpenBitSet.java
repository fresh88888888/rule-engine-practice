...
public class OpenBitSet implements BitMask {
    ...
    static {
        ...
        try {
            ...
            final Field unsaField = unsafeClass.getDeclaredField("theUnsafe");
            unsaField.setAccessible(true);
            final Object unsafe = unsaField.get(null);
    ...
    /**
     * Constructors an OpenBitSet large enough to hold <code>numBits</code>
     */
     ...
     * with bit index 0 at the least significant bit, and bit index 63 at the most
     * significant.
     * Given a bit index,
     * the word containing it is long[index/64], and it is at bit number index%64
     * within that word.
     * <p>
    ...
    /**
     * Returns the current capacity in bits (1 greater than index of the last bit)
     */
    ...
    public int prevSetBit(int index) {
        ...
        if (word != 0) {
            return (index << 6) + subIndex - Long.numberOfLeadingZeros(word);
        }

        while (--i >= 0) {
            word = bits[i];
            if (word != 0) {
                return (index << 6) + 63 - Long.numberOfLeadingZeros(word);
            }
        }
    ...
    public long prevSetBit(long index) {
        ...
        if (word != 0) {
            return ((long) (index << 6)) + subIndex - Long.numberOfLeadingZeros(word);
        }

        while (--i >= 0) {
            word = bits[i];
            if (word != 0) {
                return ((long) (index << 6)) + 63 - Long.numberOfLeadingZeros(word);
            }
        }
    ...
    public void set(long startIndex, long endIndex) {
        ...
        // since endIndex is one past the end, this is index of the last word to be
        // changed.
    ...
    /**
     * Clears a range of bits. Clearing past the end, this is index of the last.
     * 
     * @param startIndex
     * @param endIndex
     */
    public void clear(int startIndex, int endIndex) {
        ...
        int startWord = startIndex >> 6; // div 64
        if (startWord >= wlen) {
            return;
        }
        ...
        bits[startWord] &= startmask;
        int middle = Math.min(endWord, wlen);
    ...
    public void flip(long startIndex, long endIndex) {
         ...
         */
        int startmask = (int) (-1L << startIndex);
        int endmask = (int) (-1L >>> -endIndex); // 64-(endIndex&0x3f) is the same as -endIndex due to wrap
    ...
    public void union(OpenBitSet other) {
        ...
        assert (numBits = Math.max(this.numBits, other.numBits)) >= 0;
    ...
    public void xor(OpenBitSet other) {
        int newLen = Math.max(this.wlen, other.wlen);
        ensureCapacityWords(newLen);
        assert (numBits = Math.max(numBits, other.numBits)) >= 0;
    ...
    public int hashCode() {
        ...
        // zero. This effectively truncates trailing zeros without an explicit check.
        long h = 0;
    ...
    public static final class BitUtil {
        ...
        /**
         * The General Idea: instead of having an array per byte that has the offsets of
         * the next set bit, that array could be packed inside a 32 bit integer (8 4 bit
         * numbers). That should be faster than accessing an array for each index, and
         ...
         * <ul>
         * <li><code>0x43 & 0x0F</code> is 3, meaning the the first bit set is at offset
         * 3-1 = 2,</li>
        ...
        /** Return the number of set bits in an array of longs */
        ...
        /**
         * Return the popcount or cardinality of the two sets after an intersection.
        ...
        /** Return the popcount or cardianlity of A ^ B. Neither array is mofified */
        ...
        /**
         * Return the next highest power of two, or the current value if it's alreay a
