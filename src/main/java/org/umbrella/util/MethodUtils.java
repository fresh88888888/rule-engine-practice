...
public class MethodUtils {
    ...
    /*
     * This method works better than simple Clazz.getMethod in presence of
     * polymorphism.
    ...
    private static boolean isArgsNumberNotCompatible(Class[] arguments, Class<?>[] parameterTypes, boolean isVarArgs) {
        return (isVarArgs && parameterTypes.length - 1 > arguments.length)
                || (!isVarArgs && parameterTypes.length != arguments.length);
    }
    ...
    private static boolean isNumericallyCoercible(Class target, Class parameterMethod) {
        ...
        if (boxedTarget != null && Number.class.isAssignableFrom(target)) {
            if ((boxedTarget = parameterMethod.isPrimitive() ? boxPrimitive(parameterMethod)
